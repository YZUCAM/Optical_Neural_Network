...
class ComplexConv2d(nn.Module):
    def __init__(self, in_c, out_c, kernel_size, padding=0, scalar=False, bias=False, groups=1):
        ...
        self.scalar = scalar
        if self.scalar:
            self.scalar_init = torch.ones(1, dtype=torch.float32) * 0.1
            self.w_scalar = nn.Parameter(self.scalar_init)
        else:
            self.w_scalar = torch.ones(1, dtype=torch.float32).cuda()
...
class ConvBlock(nn.Module):
    def __init__(self, in_c, out_c, kernel_size=5, padding=2, scalar=False, bias=False, activation=False, groups=1, batch_norm=True,
                 kernel_complex=True, input_complex=True, pure_phase=False):
        super(ConvBlock, self).__init__()
        if kernel_complex and pure_phase:
            layers = [ComplexConv2d_PurePhase(in_c, out_c, kernel_size=kernel_size,
                                              padding=padding, scalar=scalar, bias=bias, groups=groups)]
